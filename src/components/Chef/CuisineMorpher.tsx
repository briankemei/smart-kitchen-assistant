...
export const CuisineMorpher: React.FC<CuisineMorpherProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-6 backdrop-blur-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <h2 className="text-2xl font-black text-white mt-1">Cuisine Morphing & Recipe Preview</h2>
        ...
        {selectedFilterIngredient && (
          <div className="mt-4 p-2.5 rounded-xl bg-rose-500/10 border border-rose-500/30 text-xs text-rose-300 flex items-center space-x-2">
            ...
            return (
              <button
                ...
                className={`p-3.5 rounded-2xl border text-left transition-all relative overflow-hidden group ${
                  isSelected
                    ? 'bg-gradient-to-b from-slate-800 to-slate-900 border-emerald-500 shadow-lg shadow-emerald-500/10 ring-1 ring-emerald-500'
      ...
      {deductFeedback && (
        <div className="p-4 rounded-2xl bg-emerald-950/40 border border-emerald-500/40 text-emerald-200 animate-in fade-in slide-in-from-top-2">
          <div className="flex items-center space-x-2 font-bold text-sm text-emerald-400 mb-1">
            <CheckCircle2 className="w-5 h-5 text-emerald-400" />
            <span>{deductFeedback[0]}</span>
          </div>
          <p className="text-xs text-slate-300">{deductFeedback[1]}</p>
          <p className="text-xs text-emerald-400 font-mono mt-0.5">{deductFeedback[2]}</p>
        </div>
      )}
        ...
        <div className="lg:col-span-5 space-y-4">
          <div className="bg-slate-900/70 border border-slate-800 rounded-2xl overflow-hidden backdrop-blur-sm shadow-xl">
            ...
            <div className="relative h-64 sm:h-72 w-full overflow-hidden group">
              ...
              <div className="absolute top-3 left-3 flex items-center space-x-2">
                <span className="px-2.5 py-1 rounded-full bg-slate-900/80 backdrop-blur-md text-xs font-bold text-white border border-slate-700 flex items-center gap-1.5">
                  <span>{morphedStyle.flag}</span>
              ...
              <div className="absolute bottom-3 left-3 right-3">
                ...
                <p className="text-xs text-slate-300 mt-0.5 line-clamp-1">{morphedStyle.tagline}</p>
              </div>
            </div>

            {/* Quick Metrics */}
            <div className="p-4 grid grid-cols-4 gap-2 text-center border-b border-slate-800 bg-slate-950/40">
              <div>
                <span className="text-[10px] text-slate-400 block">Calories</span>
                <span className="text-sm font-bold text-emerald-400 font-mono">{activeRecipe.calories}</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 block">Protein</span>
                <span className="text-sm font-bold text-sky-400 font-mono">{activeRecipe.macros.protein}g</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 block">Carbs</span>
                <span className="text-sm font-bold text-amber-400 font-mono">{activeRecipe.macros.carbs}g</span>
              </div>
              <div>
                <span className="text-[10px] text-slate-400 block">Fat</span>
                <span className="text-sm font-bold text-rose-400 font-mono">{activeRecipe.macros.fat}g</span>
              </div>
            </div>
            ...
            <div className="p-4 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="flex items-center justify-between bg-slate-950/50 p-2 rounded-lg border border-slate-800/80">
                  <span className="text-slate-400">Spiciness</span>
                  <span className="font-mono text-emerald-400 font-bold">
                    {'🌶️'.repeat(morphedStyle.flavorProfile.spicy) || 'Mild'}
                  </span>
                </div>
                <div className="flex items-center justify-between bg-slate-950/50 p-2 rounded-lg border border-slate-800/80">
                  <span className="text-slate-400">Tang / Citrus</span>
                  <span className="font-mono text-amber-400 font-bold">
                    {'🍋'.repeat(morphedStyle.flavorProfile.tangy)}
                  </span>
                </div>
                <div className="flex items-center justify-between bg-slate-950/50 p-2 rounded-lg border border-slate-800/80">
                  <span className="text-slate-400">Savory Umami</span>
                  <span className="font-mono text-sky-400 font-bold">
                    {'★'.repeat(morphedStyle.flavorProfile.savory)}
                  </span>
                </div>
                <div className="flex items-center justify-between bg-slate-950/50 p-2 rounded-lg border border-slate-800/80">
                  <span className="text-slate-400">Herb Aromatics</span>
          ...
          <div className="space-y-2">
            <button
              ...
              className="w-full flex items-center justify-center space-x-2 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-xs uppercase tracking-wider transition shadow-lg shadow-emerald-500/25 active:scale-98"
            >
            ...
            <button
              ...
              className="w-full flex items-center justify-center space-x-2 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 font-bold text-xs border border-slate-700 transition active:scale-98"
            >
        ...
        <div className="lg:col-span-7 space-y-4">
          ...
          <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-5 backdrop-blur-sm">
                ...
                >
                  <div>
                    <div className="flex items-center space-x-2 text-xs">
                      ...
                      <span className="font-bold text-white text-emerald-300">{swap.replacement}</span>
                    </div>
                    <p className="text-[11px] text-slate-400 mt-0.5">{swap.reason}</p>
                  </div>
                  <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 self-start sm:self-center">
                    Chef Swap
                  </span>
                </div>
              ))}
            ...
            <div className="mt-4 pt-3 border-t border-slate-800 flex flex-wrap items-center gap-2">
              <span className="text-xs text-slate-400 font-semibold">Accent Spices Needed:</span>
              {morphedStyle.accentSpices.map((spice, idx) => (
                <span
                  key={idx}
                  className="text-xs px-2.5 py-1 rounded-lg bg-slate-800 text-slate-200 border border-slate-700/80 font-medium"
                >
                  🧂 {spice}
                </span>
              ))}
          ...
          <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-5 backdrop-blur-sm">
            ...
            <div className="space-y-2">
              {morphedStyle.steps.map((step, idx) => (
                <div
                  key={idx}
                  className="flex items-start space-x-3 text-xs p-2.5 rounded-xl bg-slate-950/40 border border-slate-800/80"
                >
